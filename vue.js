...
(function (global, factory) {
  ...
  // 当前正在评估的目标监视器。这是全局唯一的，因为任何时候都可能只有一个监视器正在评估。
  // watcher being evaluated at any time.

  Dep.target = null;
  // Dep.target为公共对象
  // Dep。target存储当前的Watcher对象
  var targetStack = [];
  ...
  function protoAugment(target, src, keys) {
    ...
    // 对数组进行监控的时候 通过对数组方式变异拦截触发Dep依赖更新
    // 即把数组的更新绑定在变异的数组方法中，一旦数组发生变化从而触发更新。
   ...
   * 定义一个对象上的相对属性
   */
  //  （obj,key ，val,自定义吧警告， 是否不进行深层数据观测）


  // 当watcher进行initwatcherde的时候 会对dep。target 赋值为watch
  // 然后在watch 函数中触发get 方法 
  ...
  function defineReactive(obj, key, val, customSetter, shallow) {
    ...
    // 如果不是肤浅的 就是要进行深层观测，即数据是嵌套对象或者更为复杂的数据
    var childOb = !shallow && observe(val); //深层的Observer对象
    Object.defineProperty(obj, key, {
      enumerable: true,
      configurable: true,
      get: function reactiveGetter() {
        ...
        console.log(dep, "deppppppppppppppp", key);
        return value
      },
  ...
  // 跟刚才的接上了 通过vm的是否存在来判断 当前组件是否是祖宗组件
  // 有vm的为头组件 没有的为子组件
  ...
  if (typeof setImmediate !== 'undefined' && isNative(setImmediate) && false) {
    ...
    // 相信大家应该了解过 Web Workers，实际上 Web Workers 的内部实现就是用到了 MessageChannel，
    // 一个 MessageChannel 实例对象拥有两个属性 port1 和 port2，我们只需要让其中一个 port 监听 onmessage 事件，
    // 然后使用另外一个 port 的 postMessage 向前一个 port 发送消息即可，这样前一个 port 的 onmessage 
    // 回调就会被注册为 (macro)task，由于它也不需要做任何检测工作，所以性能也要优于 setTimeout。总之 macroTimerFunc 
    // 函数的作用就是将 flushCallbacks 注册为 (macro)task。  
  ...
  // 当更新开始时会将 flushing 变量的值设置为 true，代表着此时正在执行更新，所以根据判断条件 if (!flushing)
  //  可知只有当队列没有执行更新时才会简单地将观察者追加到队列的尾部，有的同学可能会问：“难道在队列执行更新的
  //  过程中还会有观察者入队的操作吗？”，实际上是会的，典型的例子就是计算属性，比如队列执行更新
  //  时经常会执行渲染函数观察者的更新，渲染函数中很可能有计算属性的存在，
  // 由于计算属性在实现方式上与普通响应式属性有所不同，所以当触发计算属性的 get 拦截器函数
  // 时会有观察者入队的行为，这个时候我们
  ...
  var Watcher = function Watcher(vm, expOrFn, cb, options, isRenderWatcher) {
    ...
    console.log(this.id, "odddddddddddddd")
    this.active = true;
    ...
    // watcher的get方法
    //这里调用了get方法 开始对watcher监听的数据收集watcher    实现数据监听
  };
  ...
  // initWatch => createWatcher=> $watch=> new watcher 
  ...
  function stateMixin(Vue) {
    ...
    Vue.prototype.$watch = function (expOrFn, cb, options) {
      var vm = this;
      if (isPlainObject(cb)) {
        return z(vm, expOrFn, cb, options)
      }
      options = options || {};
      options.user = true;
  ...
  // createElement用来创建一个虚拟节点。当data上已经绑定__ob__的时候，
  // 代表该对象已经被Oberver过了，所以创建一个空节点。tag不存在的时候同样创建一个空节点。
